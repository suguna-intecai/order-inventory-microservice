...
export class OrderService {
  ...
  async createOrder(input: CreateOrderInput): Promise<Order> {
    ...
    const items = this.validateOrderItems(input.items);

    // ----------------------------------------
    // 4. Combine duplicate products
    // ----------------------------------------

    const normalizedItems = this.combineDuplicateProducts(items);
    ...
    for (const item of normalizedItems) {
      try {
        ...
        throw new Error(
          `Unable to validate product ${item.productId}: ${
    ...
    for (const item of normalizedItems) {
      try {
        ...
        throw new Error(
          `Stock check failed for product ${item.productId}: ${
            error?.message || "Unknown error"
          }`,
        );
      }
    ...
    const reservedItems: CreateOrderItemInput[] = [];

    try {
      // --------------------------------------
      // Reserve stock for every product
      // --------------------------------------
      ...
      try {
        ...
        const orderItems = normalizedItems.map((item) => {
          return queryRunner.manager.create(OrderItem, {
            ...
            // Inventory gRPC currently
            // does not return product price.
            price: 0,
          });
        });
        ...
        // ------------------------------------
        // Stock reservation is now associated
        // with successfully created order.
        //
        // Do NOT release stock here.
        // ------------------------------------

        const completeOrder = await this.getOrder(savedOrder.id);
        ...
        // Prevent outer catch from releasing
        // the same stock a second time.
      ...
      // --------------------------------------
      // Reservation failed OR another error
      // occurred before database transaction.
      //
      // Release only stock that was actually
      // reserved.
      // --------------------------------------
  ...
  private combineDuplicateProducts(
    ...
    for (const item of items) {
      ...
      productMap.set(item.productId, newQuantity);
    }
  ...
  async getUserOrders(
    ...
    await this.userService.validateUser(normalizedUserId);
  ...
  async cancelOrder(orderId: number): Promise<Order> {
    ...
    const cancelledOrder = await this.getOrder(order.id);

    if (!cancelledOrder) {
      throw new Error("Order was cancelled but could not be retrieved");
    }
